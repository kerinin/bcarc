      ...
      protected    
        ...
        def parent_object
          if parent? && !parent_singleton? 
            return parent_model.find_by_param(parent_param) || 
            parent_model.find_by_param(parent_param.gsub(/_/,'-').parameterize.to_s) || 
            parent_model.find(:first, :conditions => ["legacy_permalinks like ?", "%#{parent_param}%"]) || 
            parent_model.find(:first, :conditions => ["legacy_permalinks like ?", "%#{parent_param.gsub(/_/,'-').parameterize.to_s}%"]) || 
